...
const char* mqttUser = "test5";
const char* mqttPassword = "test5";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "ESP/2/Parkour"){
   ...
   if (String(topic) == "ESP/2/TimeRace"){
    for (int i = 0; i < length; i++) {
      playerName = playerName + (char)payload[i];
    }
...
void StartParkour(){
  ...
  sendMessage("ESP/3/Parkour", textarrayplayername);
...
void StartTimeRace(){
  ...
  playerName.toCharArray(textarrayplayername, numcharsplayer);
  sendMessage("ESP/3/TimeRace", textarrayplayername);
  setred();
  recieveMessage();
  delay(5000);
  setwhite();
...
void recieveMessage(){
    ...
    if (client.connect("ESP8266Client2", mqttUser, mqttPassword )) {
      Serial.println("connected"); 
      client.subscribe("ESP/2/Parkour");
      client.subscribe("ESP/2/TimeRace");
